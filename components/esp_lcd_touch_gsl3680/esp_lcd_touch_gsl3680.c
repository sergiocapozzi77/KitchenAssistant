...
static esp_err_t touch_gsl3680_reset(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_LOGI(TAG, "=== Starting GSL3680 Reset Sequence ===");

    // GPIO Reset Toggle
    ESP_LOGI(TAG, "[1/3] Toggling reset GPIO (pin %d)", tp->config.rst_gpio_num);
    ESP_RETURN_ON_ERROR(gpio_set_level(tp->config.rst_gpio_num, 0), TAG, "GPIO set LOW failed!");
    ESP_LOGI(TAG, "  -> Reset LOW, delay 20ms");
    vTaskDelay(pdMS_TO_TICKS(20));

    ESP_RETURN_ON_ERROR(gpio_set_level(tp->config.rst_gpio_num, 1), TAG, "GPIO set HIGH failed!");
    ESP_LOGI(TAG, "  -> Reset HIGH, delay 20ms");
    vTaskDelay(pdMS_TO_TICKS(20));

    // Write to 0xe4
    ESP_LOGI(TAG, "[2/3] Writing to register 0xe4");
    addr = 0xe4;
    write_buf[0] = 0x04;
    ESP_LOGI(TAG, "  -> Data: 0x%02X", write_buf[0]);
    ret = touch_gsl3680_i2c_write(tp, addr, write_buf, 1);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "  -> FAILED! I2C write to 0x%02X returned error: 0x%x", addr, ret);
    }
    else
    {
        ESP_LOGI(TAG, "  -> SUCCESS");
    }
    ESP_LOGI(TAG, "  -> Delay 10ms");
    vTaskDelay(pdMS_TO_TICKS(10));

    // Write to 0xbc (4 bytes)
    ESP_LOGI(TAG, "[3/3] Writing 4 bytes to register 0xbc");
    write_buf[0] = 0x00;
    write_buf[1] = 0x00;
    write_buf[2] = 0x00;
    write_buf[3] = 0x00;
    ...
    ret = touch_gsl3680_i2c_write(tp, 0xbc, write_buf, 4);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "  -> FAILED! I2C write to 0xbc returned error: 0x%x", ret);
    }
    else
    {
        ESP_LOGI(TAG, "  -> SUCCESS");
    ...
    ESP_LOGI(TAG, "=== GSL3680 Reset Sequence Complete ===");
    return ESP_OK;
}
...
static esp_err_t esp_lcd_touch_gsl3680_clear_reg(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_LOGI(TAG, "Starting clear_reg sequence");

    // Reset sequence - INCREASED DELAYS
    ESP_LOGI(TAG, "Toggling reset GPIO (pin %d)", tp->config.rst_gpio_num);
    ESP_RETURN_ON_ERROR(gpio_set_level(tp->config.rst_gpio_num, 0), TAG, "GPIO set level LOW error!");
    ESP_LOGI(TAG, "Reset LOW, waiting 50ms"); // Increased from 20ms
    vTaskDelay(pdMS_TO_TICKS(50));

    ESP_RETURN_ON_ERROR(gpio_set_level(tp->config.rst_gpio_num, 1), TAG, "GPIO set level HIGH error!");
    ESP_LOGI(TAG, "Reset HIGH, waiting 100ms"); // Increased from 20ms to 100ms
    vTaskDelay(pdMS_TO_TICKS(100));             // GSL3680 needs time to boot!

    // Register 0x88
    addr = 0x88;
    wrbuf[0] = 0x01;
    ESP_LOGI(TAG, "Writing 0x%02X to reg 0x%02X", wrbuf[0], addr);
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp, addr, wrbuf, 1), TAG, "Write to 0x88 failed");
    ...
    wrbuf[0] = 0x04;
    ESP_LOGI(TAG, "Writing 0x%02X to reg 0x%02X", wrbuf[0], addr);
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp, addr, wrbuf, 1), TAG, "Write to 0xe4 failed");
    vTaskDelay(pdMS_TO_TICKS(10)); // Increased from 5ms
    ...
    wrbuf[0] = 0x00;
    ESP_LOGI(TAG, "Writing 0x%02X to reg 0x%02X", wrbuf[0], addr);
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp, addr, wrbuf, 1), TAG, "Write to 0xe0 failed");
    vTaskDelay(pdMS_TO_TICKS(20));

    ESP_LOGI(TAG, "clear_reg sequence complete");
    return ESP_OK;
}
