...
std::vector<RecipeSuggestion> RecipeGialloZafferanoService::getRecipeSuggestions(
    ...
    int page)
{
    ESP_LOGI(TAG, "Calling Appwrite function for Giallo Zafferano search (page %d)", page);
    ...
    cJSON_AddStringToObject(payload, "source", "giallozafferano");
    cJSON_AddNumberToObject(payload, "page", page);

    // Optional parameters (the function expects them even if ignored)
...
std::vector<RecipeSuggestion> RecipeGialloZafferanoService::parseSuggestionsResponse(const std::string &envelope)
{
    ...
    ESP_LOGI(TAG, "Received %d Giallo Zafferano suggestions", count);
