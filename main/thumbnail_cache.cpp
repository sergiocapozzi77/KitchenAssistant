...
enum class JobType : uint8_t { PUT, GET };
...
static bool load_index_internal()
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!jh || !ju || !jw || !jht || !jts || !jsz) continue;
...
{
    if (s_cache.size() <= max_items) return;
...
static void cache_io_task(void *arg)
{
    ...
    IoJob job;
    while (true) {
        if (xQueueReceive(s_io_queue, &job, portMAX_DELAY) != pdTRUE) continue;
            ...
            if (f) {
                ...
                if (written == job.jpeg_len) {
                    ...
                    auto it = std::find_if(s_cache.begin(), s_cache.end(),
                        [&job](const CacheEntry &e){ return e.hash == job.hash; });
                    ...
                    save_index_internal();
                    ESP_LOGI(TAG, "Cached: %s (%zu B)", job.hash, job.jpeg_len);
         ...
         std::vector<uint8_t> &out_jpeg)
{
    ...
    bool exists = std::any_of(s_cache.begin(), s_cache.end(),
        [&hash](const CacheEntry &e){ return e.hash == hash; });
    xSemaphoreGive(s_mutex);
    ...
    if (result) ESP_LOGI(TAG, "Cache hit: %s", hash.c_str());
    return result;
}
